...
To avoid a second low, you can instruct Trio not to give any SMBs for a while after a low, giving your BG time to stabilize. Set up a [Temporary Target](../usage/features/temp-targets.md) preset that is a bit higher than your usual target (e.g., 7,5 mmol/L | 136 mg/dl). Ensure that [Allow SMB With High Temptarget](../configuration/settings/algorithm/smb-settings.md#allow-smb-with-high-temptarget) is disabled. Make the temporary target last for a while (e.g., 30-45 min).
