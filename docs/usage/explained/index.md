# Explain It To Me

![Trio Logo](../../assets/images/trio-logo.png){ .skip-lightbox width="75" align="left" }

Welcome to the Explain It To Me homepage! Here you will find a variety of items that at first glance can be very confusing. We hope this page will help them make sense!  

- - -

Please use the navigation menu :fontawesome-solid-bars:, search bar :fontawesome-solid-magnifying-glass:, or the menu below to find the section you are looking for.

<div class="grid cards" markdown>

-   __[Using Dynamic ISF](dynamic-isf.md)__

    - - -
 
    A series of tips, tricks, and best practices for using Dynamic ISF in Trio

-   __[Loop Status](current-loop-status-explained.md)__

    - - -

    How to read the loop status pills in Trio and Nightscout

-   __[Desmos Graphs](desmos-explained.md)__

    - - -
 
    How to read the Desmos Graphs when adjusting your Dynamic ISF Settings
    
</div>
