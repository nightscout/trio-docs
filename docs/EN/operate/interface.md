...
The looping indicator will be 
:::{div} sd-text-secondary
grey
:::
 if no data is present. It will be 
 :::{div} sd-text-success
 green
 :::
 when it has been less than 5 minutes since a successful loop cycle. It will turn 
 :::{div} sd-text-warning
 yellow
 :::
  if no loop cycle has been completed for over 5 minutes. If more than 10 minutes go by without a successful loop cycle, the loop circle will turn 
  :::{div} sd-text-danger
  red
  :::
  . The section to the left of the loop circle shows you the approximate units of insulin left in your pump. If you're using Omnipod, 50+ U means more than 50 units of insulin are left. Below the insulin left, you will see your pump battery status if you're using a Medtronic pump and the days/hours left if you're using an Omnipod.
