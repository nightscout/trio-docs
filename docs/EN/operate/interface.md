...
The looping indicator will be {bdg-secondary}`grey` if no data is present. It will be {bdg-success}`green}` when it has been less than 5 minutes since a successful loop cycle. It will turn {bdg-warning}`yellow` if no loop cycle has been completed for over 5 minutes. If more than 10 minutes go by without a successful loop cycle, the loop circle will turn {bdg-danger}`red`. The section to the left of the loop circle shows you the approximate units of insulin left in your pump. If you're using Omnipod, 50+ U means more than 50 units of insulin are left. Below the insulin left, you will see your pump battery status if you're using a Medtronic pump and the days/hours left if you're using an Omnipod.
...
<img src="https://github.com/nightscout/Open-iAPS-docs/assets/31315442/19e80314-b34e-4815-b81f-189d35c2fe39" width="400"/>
