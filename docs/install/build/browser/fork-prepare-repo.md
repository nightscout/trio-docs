...
!!! question "Important Decision: Personal Account or Organization?"
    ...
    **📖 Haven't decided?** Review [Create a Free GitHub Organization](create-github-account.md#create-a-free-github-organization){: target="_blank"} first, then come back here.
...
???+ abstract "Section Summary (click to open/close)"
    Fork [https://github.com/nightscout/Trio](https://github.com/nightscout/Trio) into your account.

    [<span class="loop-bigger">:material-skip-forward:</span>](actions/validate-secrets.md) To skip the detailed instructions, click on [Validate <code>Secrets</code>](actions/validate-secrets.md)

!!! warning "Existing Fork"
    If you already have a fork of <code>Trio</code>, click on [Already Have Trio](#already-have-trio) to decide what to do. That section provides links to return you to these instructions.

### Create the `Fork`

1. Click this link [https://github.com/nightscout/Trio](https://github.com/nightscout/Trio) to open the <code>Trio</code> repository owned by `nightscout`
...
1. Refer to the GIF below - note this GIF is for LoopWorkspace - but the actions you take are the same for Trio:
    ...
    * Click on the green `Create fork` button
    ...
    ![fork information for Trio](img/fork-org.gif){width="700"}
    {align="center"}
...
![after creating the fork Trio](img/new-trio-fork.png){width="700"}
{align="center"}
...
1. Return to your forked copy of <code>Trio</code>
...
1. Click on <code>Trio</code> to open that repository
    * You should see a bar of icons across the top with Settings on the far right

        ![settings screen](img/gh-settings-logged-in.svg){width="700"}
        {align="center"}

1. Click on the Settings Icon near the top right of your Trio
    * If you don't see ⚙️ `Settings`, make your browser wider or scroll to the right
    * If you still don't see ⚙️ `Settings`, then you are **not** on your fork or you need to sign in to your *GitHub* account
    * After you click on ⚙️ `Settings`, the left side of your screen should resemble the graphic below

        ![settings screen](img/gh-settings.svg){width="300"}
        {align="left"}
...
Enter the name of each <code>Secret</code> found in [Save Your Information](intro-secrets.md#save-your-information) and your value for that `Secret`.
...
**→ [Validate Secrets](actions/validate-secrets.md)**
...
## Already Have&nbsp;<span translate="no">Trio</span>?

Some people may already have a copy (`fork`) of <code>Trio</code>.

If your copy (`fork`) is **not** from `nightscout`, follow the [Delete and Start Fresh](#delete-and-start-fresh) directions.

If your copy (`fork`) is from `nightscout`:

* Open your <code>Trio</code> repository (`https://github.com/my-name-org/Trio`) where you use your version of `my-name` in the URL
...
* Return to [Fork Trio](#fork-trio) and follow all the instructions
